...
(function() {
    ...
    class helper {
        ...
        constructor(){
            ...
            this.mappings = [
                ...
                { code: "br", area: "Africa",       country: "Brazil" },
    ...
    class styleHandler{
        ...
        addPageStyle() {
            let pageStyle = `
                ...
                profile .settings-content .content-holder b {
                    color: snow;
                }
        ...
        addCardStyle() {
            let cardStyle = `
                ...
                .portfolio-content .card .card-content .coordinates,
                .portfolio-content .card .card-content .tileClass {
                    color: snow;     
                }          
    ...
    class profilePageUpgrader {
        ...
        checkProfilePageUpgrade(){
            if(this.helper.isAvailable(window.location.hash) && window.location.hash.includes("profile")){
                ...
                if(window.location.hash.replace("#profile/","") !== riot.auth0user.id){
                    console.warn("not current user");    
                } else {
                    let profileElement = document.querySelector(this.selector);
                    if(!this.helper.isAvailable(profileElement)){
                        this.helper.showCustomToast("error", `Could not get profile! ${this.notifyDevMessage}`);
                    } else {
                        let riotTag = profileElement._tag;
                        if(!this.helper.isAvailable(riotTag)){
                            this.helper.showCustomToast("error", `Could not get tag! ${this.notifyDevMessage}`);
                        } else {
                            if(!this.helper.isAvailable(riotTag.mihaj)){
                                this.customiseProfilePage(riotTag);	
                            }
                        }
                    } 
                }
        ...
        customiseProfilePage(riotTag) {
            ...
            ;(async () => {
                ...
                let cachedSuccess = await this.cacheProperties(riotTag);
                if(!cachedSuccess){
                    let maxRetry = 10;
                    let retryCount = 1;
                    while(retryCount < maxRetry){
                        console.log(`retry property caching ${retryCount}/${maxRetry}`);
                        cachedSuccess = await this.cacheProperties(riotTag);
                        if(cachedSuccess){
                            break;
                        } else {
                            retryCount++;
                ...
                if(!cachedSuccess){
                    ...
                    this.removeReaddFilters(riotTag);
                    this.addExtraHtml();
        ...
        removeReaddFilters(riotTag) {
            try {
                ...
                this.addDisplayTypeDropdown(riotTag);
                this.addOrderingDropdown(riotTag);
                this.addDisplayModeSwitch(riotTag);
                this.addExportLink();
        ...
        addDisplayTypeDropdown(riotTag) {
            try {
                ...
                if(newPropertiesData.length === 0){
                    ...
                    newPropertiesData.forEach(propertyData => {
                        let newCard = this.cardTemplate
                                                ...
                                                .replaceAll("#LOCATION#", propertyData.location)
    ...
    setTimeout(() => {
        try {
            ...
            let profilePageUpgraderInstance = new profilePageUpgrader(helperInstance);
