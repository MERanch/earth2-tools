...
// @author       Mihaly Szolnoki -> E2: MihajA414 - MSZY5BLXAP -> discord: mihaj#5170
// @match        https://app.earth2.io/
// @grant        none
// @license MIT
// @currentversion	0.3.0 : Added a more unified structure (compared to other scripts). Adjusted for bazaar sales. 3rd party link is still removed
// ==/UserScript==
...
(async function () {
    ...
    class Event {
        ...
        Trigger(targetSelector, eventType, eventData, bubbles) {
            try {
                let event;

                if (window["CustomEvent"]) {
                    event = new CustomEvent(eventType, { detail: { eventData: eventData }, bubbles: bubbles });
                } else {
                    event = document.createEvent("CustomEvent");
                    event.initCustomEvent(eventType, true, true, { eventData: eventData });
                }

                let targets = Array.from(document.querySelectorAll(targetSelector));
                if(targetSelector === "window"){
                    targets = [window];
                }

                //console.warn(`       Trigger @ [${targetSelector}] >> [${eventType}]`);
                if (targets.length > 0) {
                    targets.forEach(item => {
                        item.dispatchEvent(event);
                    });
                } else {
                    console.warn(`trigger: for selector [${targetSelector}] no results (event: ${eventType})`);
                }
            } catch (ex) {
                console.log(ex);
            }
        }
        Listen(targetSelector, eventTypes, handler, useCapture) {
            try {

                if (!this.EventListeners.some(e => e.targetSelector === targetSelector && e.eventTypes === eventTypes && e.handler === handler.toString())) {
                    //console.warn(`       listen [${targetSelector}] types [${eventTypes}]`)//, handler);
                    this.EventListeners.push({ targetSelector, eventTypes, handler: handler.toString() });
                    let targets = document.querySelectorAll(targetSelector);
                    if (targetSelector === "window") {
                        targets = [window];
                    } else if (targetSelector === "document") {
                        targets = [document];
                    }
                    if (targets.length > 0) {
                        let eventTypeArray = eventTypes.split(" ");

                        if (eventTypeArray.length > 0) {
                            eventTypeArray.forEach(eventType => {
                                targets.forEach(item => {
                                    item.addEventListener(eventType, handler, useCapture);
                                });
                            });
                        }
                    } else {
                        console.warn(`[Listen]: no elements for selector [${targetSelector}]`, eventTypes);
                    }
                }
            } catch (ex) {
                console.log(ex);
    ...
    class E2API {
        ...
        async getTransactionPage(pageNumber) {

            let offset = (pageNumber - 1) * this.itemsPerPage;
            let url = `api/v2/my/balance_changes/?limit=${this.itemsPerPage}&offset=${offset}`;
        ...
        async getAllTransactions() {
            let firstPageData = await this.getTransactionPage(1);
            ...
            for (let i = 1; i < pageCount; i++) {
                ...
                event.Trigger("window", "update-transaction-button", { text: `query page ${pageNumber - 1} / ${pageCount}` });
                await helper.sleep(150);

                //updateTransactionExportButtonText(`query page ${pageNumber-1} / ${pageCount}`);
                let pageData = await this.getTransactionPage(pageNumber);
            ...
            if (failedPages.length > 0) {
                ...
                while (retryCount < maxRetry && failedPages.length > 0) {
                    ...
                    let pageData = await this.getTransactionPage(currentPage);
                    if (helper.isAvailable(pageData)) {
                        window.transactions = window.transactions.concat(pageData.results);
    ...
    class TransactionCSVExport {
        ...
        async generateExport() {
            ...
            for (let i = 0; i < window.transactions.length; i++) {
                let transaction = window.transactions[i];
                ...
                let thirdPartyLink = "";
                let jewelData = "";
                if (helper.isAvailable(transaction.linked_object)) {
                    let linked = transaction.linked_object;

                    switch (linked.model_name.toLowerCase()) {
                        case "jewel":
                            let jewelColor = JewelData.getColorName(linked.color);
                            jewelData = `${JewelData.getSize(linked.size)} | ${jewelColor} (${JewelData.getEffect(linked.size, jewelColor)})`
                            break;
                        case "landfield":
                            if (helper.isAvailable(linked.location)) {
                                loc = helper.cleanString(linked.location).split(",").reverse().join(" | ");
                            }
                            if (helper.isAvailable(linked.description)) {
                                desc = helper.cleanString(linked.description).replaceAll(",", " | ");
                            }
                            if (helper.isAvailable(linked.id)) {
                                // let propertyInfo = await this.getPropertyInfo(linked.id);
                                // if (!helper.isAvailable(propertyInfo)) {
                                //     console.warn(`No property infor for transaction: `, transaction);
                                // } else {
                                //     if (propertyInfo.owner.id !== window.auth0user.id) {
                                //         thirdPartyName = propertyInfo.owner.username;
                                //         thirdPartyLink = `https://app.earth2.io/#profile/${propertyInfo.owner.id}`;
                                //     }
                                //     
                                // }
                                link = `https://app.earth2.io/#propertyInfo/${transaction.linked_object.id}`;
                            }
                            break;
                        default:
                            console.warn(`unknown model name [${linked.model_name}]`, t);
                            break;
                    }
                }

                let dateFormatted = transaction.created_display.split("/").reverse().join("-");

                let transDataCSV = `${dateFormatted},${transaction.balance_change_type_display},${transaction.amount},${transaction.balance_before},${transaction.balance_after}`;
                let propDataCSV = `${loc},${desc},${link}`;

                allTransactions.push(`${transDataCSV},${propDataCSV},${jewelData}`);
            }

            let shamelessPlug = `If you want to thank me ${Strings.NEWLINE}use my code -->MSZY5BLXAP ${Strings.NEWLINE}(or tip me Paypal: csimbum@gmail.com)${Strings.NEWLINE}`
            let transactionCSVHeader = `Created on,Change Type,Amount,Balance before,Balance after,Location,Description,Link, jewel data${Strings.NEWLINE}`;

            window.allTransactionsCSV = shamelessPlug + transactionCSVHeader + allTransactions.join(Strings.NEWLINE);

            helper.createDownloadFile("transactions", window.allTransactionsCSV);
        }

        async getPropertyInfo(propertyId, index) {
            if (!this.propertiesCache.has(propertyId)) {
                let propertyInfo = await api.getPropertyInfoShort(propertyId);
                if (helper.isAvailable(propertyInfo)) {
                    console.log(`[${index}] adding new property (current count: ${this.propertiesCache.size}) : `, propertyInfo)
                    this.propertiesCache.set(propertyId, propertyInfo);
                }
            }

            return this.propertiesCache.get(propertyId);
        }
    }
    ...
    class TransactionUI {
        constructor() {
            this.transactionExportButtonName = "transaction_export_button";
            this.transactionExportButtonSelector = `.settings-header .content #${this.transactionExportButtonName}`;
            this.exportButtonText = "export";
            this.csvExport = new TransactionCSVExport();
        }
        ...
        checkTransactionExportButton() {
            if (this.isTransactionsPage()) {
                let exportButton = document.querySelector(`.settings-header a#${this.transactionExportButtonName}`);

                if (!helper.isAvailable(exportButton)) {
                    //console.log("tr. export button not present");
                    this.addTransactionExportButton();
                } else {
        ...
        addTransactionExportButton() {
            ...
            let exportButtonTag = `
                <div class="extra-bits" id=${this.transactionExportButtonName}>
                    <button type="button" style="margin-right: 20px; font-size:20px;"><span style="padding-left: 10px;padding-right: 10px;color: #a0a;">${this.exportButtonText}</span></button>
                </div>
            `;
            ...
            setTimeout(() => {
                ...
                event.Listen("window", "update-transaction-button", (e) => {
                    //console.log("event: ", e);
                    this.updateTransactionExportButtonText(e.detail.eventData.text);
                })

                document.querySelector(this.transactionExportButtonSelector).addEventListener("click", async () => {
                    if (window.exportButtonDisabled === false) {
                        window.exportButtonDisabled = true;
                        document.querySelector(this.transactionExportButtonSelector).disabled = true;
                        this.updateTransactionExportButtonText("Please wait...");

                        await api.getAllTransactions();
                        await this.csvExport.generateExport();

                        this.updateTransactionExportButtonText(this.exportButtonText);
                        document.querySelector(`.settings-header #${this.transactionExportButtonName}`).disabled = false;
                        window.exportButtonDisabled = false;
                    }
                }, false);
            }, 1500);
        ...
        updateTransactionExportButtonText = (text) => {
            document.querySelector(`${this.transactionExportButtonSelector} span`).textContent = text;
        }
    }

    let ui = new TransactionUI();
    ui.checkTransactionExportButton();
